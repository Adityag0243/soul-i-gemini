...
"""
souli_pipeline/voice/run_worker.py  — v2 (Gemini edition)

Entry point for the LiveKit voice worker.

Local usage:
    python -m souli_pipeline.voice.run_worker

EC2 Docker usage (via docker-compose):
...
def main():
    ...
    session_id = os.environ.get("VOICE_SESSION_ID", f"voice_{os.urandom(4).hex()}")
    logger.info("Starting Souli Voice Worker — session: %s", session_id)

    agent = SouliVoiceAgent(session_id=session_id)
    agent.start()
